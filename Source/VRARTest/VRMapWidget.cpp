...
void UVRMapWidget::updatePlayerMarker(int targetIndex)
{
    ...
    if (!MarkerSlot || !TargetSlot)
    {
        GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, TEXT("Can't be cast"));
    ...
    FString DebugString = FString::Printf(TEXT("Target Pos: (X=%.2f, Y=%.2f)"), TargetPosition.X, TargetPosition.Y);
    GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Cyan, DebugString);
}
