...
void AVRCharacter::OverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	else if (OverlappedComponent == leftControllerHammerCollider || OverlappedComponent == rightControllerHammerCollider)
	{
		...
		{
			ASpellPooledEntity *collidedProjectile = Cast<ASpellPooledEntity>(OtherActor);
			if (collidedProjectile && OtherComp->IsA(UBoxComponent::StaticClass()))
			{
				collidedProjectile->onTimeToLiveExpired();
			}
		}
	}
}
